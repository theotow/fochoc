...
func TestScraper(t *testing.T) {
	initLogger(true)
	SkipConvey("getER20Tokens() should execute without error", t, func() {
	...
	SkipConvey("getER20Tokens() should return empty map if address invalid", t, func() {
		res, _ := getER20Tokens("ETH", EthAddress+"32233")
		So(len(res), ShouldEqual, 0)
	})
	SkipConvey("getCoins() should execute without error", t, func() {
		res := getCoins(1, 100)
		So(res["BTC"].Symbol, ShouldEqual, "BTC")
		So(res["BTC"].BtcPrice, ShouldEqual, 1)
		So(res["BTC"].UsdPrice, ShouldBeGreaterThan, 0)
	})
	SkipConvey("getCoinsAsync() should execute without error", t, func() {
		res := getCoinsAsync()
		So(res["BTC"].Symbol, ShouldEqual, "BTC")
		So(res["BTC"].BtcPrice, ShouldEqual, 1)
	...
	SkipConvey("getBalanceChainz() should execute without error", t, func() {
		res, _ := getBalanceChainz("BTC", BtcAddress)
		So(res["BTC"], ShouldEqual, 0.00996768)
	})
	SkipConvey("getBalanceChainz() should execute have error", t, func() {
		_, err := getBalanceChainz("BTCCC", BtcAddress)
		So(err.Error(), ShouldEqual, "parse float blanace of BTCCC failed")
	})
	SkipConvey("asyncRun() should do success / error case", t, func() {
		// success case
	...
	SkipConvey("resolveCoins() should run without error", t, func() {
		array := []BalanceSimple{
			{
				Address:  EthAddress,
				Currency: "ETH",
